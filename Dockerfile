...
# a digest-pinnable base, an Alpine refresh, and OCI labels. See README.md for
# design notes.
#
# Build:
#   docker build \
#     --build-arg FLARUM_VERSION=1.8.19 \
#     -t ghcr.io/pikapods/docker-flarum:1.8.19 .
#
...
ARG ALPINE_VERSION
# Plain semver, no leading "v", passed to Composer as-is: Composer normalises
# the constraint, so this matches whether upstream tagged the release v1.8.18
# or 1.8.19 (it has done both). Never reintroduce a hardcoded "v" — assuming
# that prefix is what hid 1.8.19's availability in the first place.
...
LABEL org.opencontainers.image.title="Flarum" \
      org.opencontainers.image.description="Self-maintained Flarum container" \
